...
<div class="max-w-6xl mx-auto py-6 px-2 sm:px-4" style="background: linear-gradient(135deg, rgb(38,186,165) 0%, rgb(55,95,122) 100%); border-radius: 18px; box-shadow: 0 4px 24px 0 rgba(55,95,122,0.10);">
    <h1 class="text-2xl sm:text-3xl font-bold mb-4 sm:mb-6 flex items-center gap-2" style="color: #375F7A;">
        <i class="fas fa-map-marked-alt"></i> Colegios más aplazados por distrito ({{ $anio }})
    </h1>

    <form method="GET" action="{{ url('/distritos-aplazados') }}" class="bg-white rounded-xl p-4 sm:p-6 mb-6 shadow">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                <label class="block text-sm font-semibold text-secondary-700 mb-1">Ciudad (municipio)</label>
                <input
                    list="municipios-list"
                    name="municipio"
                    value="{{ $municipio }}"
                    placeholder="Buscar ciudad..."
                    class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary-400 focus:outline-none"
                />
            ...
            <div>
                <label class="block text-sm font-semibold text-secondary-700 mb-1">Distrito</label>
                <select
                    name="distrito"
                    class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary-400 focus:outline-none"
                    {{ $distritos->isEmpty() ? 'disabled' : '' }}
                >
                    <option value="">Todos los distritos</option>
                    @foreach($distritos as $dist)
                        <option value="{{ $dist }}" @selected($distrito === $dist)>{{ $dist }}</option>
                    @endforeach
                </select>
                @if($municipio && $distritos->isEmpty())
                    <p class="text-xs text-red-600 mt-1">La ciudad seleccionada no tiene distritos disponibles.</p>
                @endif
            </div>
            <div>
        ...
        <div class="mt-4 flex flex-wrap gap-2">
            ...
            @if($municipio || $distrito)
                <a href="{{ url('/distritos-aplazados') }}" class="px-4 py-2 rounded-lg font-semibold" style="background:#eef2f7;color:#375F7A;">
                    Limpiar
                </a>
            @endif
        </div>
    ...
    @if(!$municipio)
        <div class="bg-yellow-100 text-yellow-800 rounded-lg p-6 text-center">
            Selecciona una ciudad para ver el ranking por distrito.
        </div>
    @else
        <div class="bg-white rounded-xl shadow p-4 sm:p-6 mb-6">
            <h2 class="text-lg sm:text-xl font-bold mb-3" style="color:#375F7A;">
                Colegio con mayor reprobación en cada distrito
            </h2>
            @if($rankingPorDistrito->isEmpty())
                <div class="bg-yellow-100 text-yellow-800 rounded-lg p-4 text-center">
                    No hay datos de reprobados para la ciudad seleccionada en esta gestión.
                </div>
            @else
                @php
                    ...
                    <table class="min-w-full bg-white rounded-xl shadow text-xs sm:text-base">
                        <thead style="background: #26BAA5; color: #fff;">
                            <tr>
                                ...
                                <th class="py-2 sm:py-3 px-2 sm:px-4 text-left whitespace-nowrap">Distrito</th>
                                ...
                                <th class="py-2 sm:py-3 px-2 sm:px-4 text-center whitespace-nowrap">
                                    <a class="underline" href="{{ url('/distritos-aplazados?' . http_build_query(array_merge($baseQuery, ['orderBy' => 'reprobados', 'orderDir' => $toggleReprobados]))) }}">Reprobados {{ $orderBy === 'reprobados' ? ($orderDir === 'asc' ? '↑' : '↓') : '' }}</a>
                                </th>
                                <th class="py-2 sm:py-3 px-2 sm:px-4 text-center whitespace-nowrap">Matriculados</th>
                                <th class="py-2 sm:py-3 px-2 sm:px-4 text-center whitespace-nowrap">
                                    <a class="underline" href="{{ url('/distritos-aplazados?' . http_build_query(array_merge($baseQuery, ['orderBy' => 'porcentaje', 'orderDir' => $togglePorcentaje]))) }}">% Reprobación {{ $orderBy === 'porcentaje' ? ($orderDir === 'asc' ? '↑' : '↓') : '' }}</a>
                                </th>
                            ...
                            @foreach($rankingPorDistrito as $item)
                                <tr style="border-bottom: 1px solid #e5e7eb; background: {{ $loop->even ? 'rgba(38,186,165,0.08)' : '#fff' }};">
                                    ...
                                    <td class="py-1 sm:py-2 px-2 sm:px-4 text-center">
                                        <a href="{{ url('/distritos-aplazados?municipio=' . urlencode($municipio) . '&distrito=' . urlencode($item->distrito) . '&gestion=' . $anio) }}"
        ...
        @if($distrito)
            <div class="bg-white rounded-xl shadow p-4 sm:p-6">
                <h2 class="text-lg sm:text-xl font-bold mb-3" style="color:#375F7A;">
                    Ranking de colegios del distrito: {{ $distrito }}
                </h2>
                        ...
                        <table class="min-w-full bg-white rounded-xl shadow text-xs sm:text-base">
                            <thead style="background: #26BAA5; color: #fff;">
                                <tr>
                                    ...
                                    <th class="py-2 sm:py-3 px-2 sm:px-4 text-left whitespace-nowrap">Colegio</th>
                                    <th class="py-2 sm:py-3 px-2 sm:px-4 text-center whitespace-nowrap">
                                        <a class="underline" href="{{ url('/distritos-aplazados?' . http_build_query(array_merge($baseQueryDetalle, ['orderByDetalle' => 'reprobados', 'orderDirDetalle' => $toggleReprobadosDetalle]))) }}">Reprobados {{ $orderByDetalle === 'reprobados' ? ($orderDirDetalle === 'asc' ? '↑' : '↓') : '' }}</a>
                                    </th>
                                    <th class="py-2 sm:py-3 px-2 sm:px-4 text-center whitespace-nowrap">Matriculados</th>
                                    <th class="py-2 sm:py-3 px-2 sm:px-4 text-center whitespace-nowrap">
                                        <a class="underline" href="{{ url('/distritos-aplazados?' . http_build_query(array_merge($baseQueryDetalle, ['orderByDetalle' => 'porcentaje', 'orderDirDetalle' => $togglePorcentajeDetalle]))) }}">% Reprobación {{ $orderByDetalle === 'porcentaje' ? ($orderDirDetalle === 'asc' ? '↑' : '↓') : '' }}</a>
                                    </th>
